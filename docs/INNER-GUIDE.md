...
> **目前状态(2026-09):休眠。**
> `#/compass` 已改回以指南盘为中心的五段版面(01 我的内在指南 → 02 想留给自己的几句话
> → 03 此刻的我 → 04 今天想问自己的一个问题 → 05 我的星空记录),这一页**不再渲染**方向指南。
> 三支 `assets/guide-*.js` 模组、`window.Guide` 储存层与使用者装置上既有的
> `inner_sky_guide_profile_v1` / `inner_sky_guide_history_v1` **原样保留,不迁移、不删除、不读写**。
> 05 的记录只读 `Compass.store`(`compass_entries`),不合并 Guide.history。
> 下面的规格描述的是模组本身,仍然有效;要不要给它一个自己的路由,是之后的产品决定。
...
这一份规格原本把 `#/compass` 整页换成方向指南(PR #27)。之后的版面决定把那一页改回
「认识自己」的五段:四个方向、想留给自己的几句话、今天的问题、此刻的我、星空记录 ——
所以方向指南**目前没有画面在用**。模组、储存层与使用者的历史资料全部保留(见页首)。
